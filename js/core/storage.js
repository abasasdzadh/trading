 ...
 *  - localStorage:  credentials (persisted across sessions for user convenience)
...
const STORE_NAMES = Object.freeze([
  ...
  'paperTrades',
  'signals',
  // NOTE: credentials are stored in localStorage for persistence.
]);
...
export class StorageService {
  ...
  // -------------------------------------------------------------------------
  // Credentials (localStorage — persisted across sessions)
  // -------------------------------------------------------------------------

  /**
   * Get a stored credential value.
   * Credentials live in localStorage so they persist across page refreshes.
   *
  ...
  getCredentials(providerId) {
    ...
    const raw = localStorage.getItem(`ta_cred_${providerId}`);
  ...
  setCredentials(providerId, data) {
    if (!isBrowser()) return;
    localStorage.setItem(`ta_cred_${providerId}`, JSON.stringify(data));
  }

  /**
   * Remove credentials for a provider from localStorage.
   *
   * @param {string} providerId
   */
  removeCredentials(providerId) {
    if (!isBrowser()) return;
    localStorage.removeItem(`ta_cred_${providerId}`);
  }
    ...
    if (!excludeCredentials && isBrowser()) {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('ta_cred_')) {
          ...
          credentials[providerId] = parseJSONSafe(localStorage.getItem(key));
        }
      }
  ...
  async importData(data) {
    ...
    // Credentials — import into localStorage
