...
class App {
  ...
  async init() {
    try {
      ...
      // Listen for settings changes (e.g. theme, language)
      this.eventBus.on('settings:changed', (data) => {
        if (data.key === 'theme' || (data.section === 'general' && this.settings.theme)) {
  ...
  async _initProviders() {
    // Market provider
    const marketCreds = JSON.parse(localStorage.getItem('ta_market_credentials') || '{}');
    ...
    // AI provider
    const aiCreds = JSON.parse(localStorage.getItem('ta_ai_credentials') || '{}');
    if (aiCreds.apiKey) {
      try {
  ...
  _handleRoute(ctx) {
    ...
    document.querySelectorAll('.sidebar-item').forEach(item => {
      ...
      if (navPath && (ctx.path === navPath || ctx.path.startsWith(navPath + '/'))) {
  ...
  _showDemoBanner() {
    ...
    container.innerHTML = `
      <div class="demo-banner" id="demo-banner">
        <span class="demo-banner-icon"><svg style="width:16px;height:16px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></span>
  ...
  generateSignalFromIndicators(candles, indicators) {
  ...
  setMarketProvider(type, config) {
    ...
    localStorage.setItem('ta_market_credentials', JSON.stringify({ type, ...config }));
  ...
  setAIProvider(type, config) {
    ...
    localStorage.setItem('ta_ai_credentials', JSON.stringify({ type, ...config }));
  ...
  _applyTheme(theme) {
    ...
    if (meta) {
      meta.content = theme === 'light' ? '#f8fafc' : '#0f1117';
    }
  }
}
