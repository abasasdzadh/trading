...
function _settingsTabs() {
  ...
  return `<div class="tabs-container" id="settings-tabs">
    <div class="tabs-nav">${tabItems.map((tab, i) => {
      const tabKey = tab.toLowerCase();
      const label = t('settings.tab.' + tabKey);
      return `<button class="tab-item${i === 0 ? ' active' : ''}" data-tab="${tabKey}">${label}</button>`;
    }).join('')}</div>
...
function _generalPanel() {
  ...
  return `<div class="card"><div class="card-body">
    <div class="form-row">
      <div class="form-group"><label class="form-label">${t('settings.theme')}</label>
        <select class="select" data-setting="theme"><option value="dark">${t('settings.theme.dark')}</option><option value="light">${t('settings.theme.light')}</option></select></div>
      <div class="form-group"><label class="form-label">${t('settings.language')}</label>
        <select class="select" data-setting="language"><option value="en">${t('settings.lang.en')}</option><option value="fa">${t('settings.lang.fa')}</option></select></div>
      <div class="form-group" style="display:flex;align-items:flex-end;padding-bottom:0">
        ...
        <label class="form-label" style="margin:0 0 0 8px;cursor:pointer">${t('settings.demoMode')}</label></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">${t('settings.defaultSymbol')}</label>
        <input class="input" data-setting="defaultSymbol" value="BTCUSDT" placeholder="BTCUSDT"></div>
      <div class="form-group"><label class="form-label">${t('settings.defaultTimeframe')}</label>
    ...
    <div class="form-row">
      <div class="form-group" style="display:flex;align-items:flex-end;padding-bottom:0">
        ...
        <label class="form-label" style="margin:0 0 0 8px;cursor:pointer">${t('settings.autoRefresh')}</label></div>
      <div class="form-group"><label class="form-label">${t('settings.refreshInterval')}</label>
        <input class="input" type="number" data-setting="refreshInterval" value="30" min="5"></div>
    </div>
    <div style="margin-top:var(--space-4)"><button class="btn btn-primary" id="save-general">${t('settings.saveGeneral')}</button></div>
  </div></div>`;
}

function _providersPanel() {
  return `<div style="display:flex;flex-direction:column;gap:var(--space-4)">
  <div class="card"><div class="card-header"><div class="card-header-title">${t('settings.marketProvider')}</div></div><div class="card-body">
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">${t('settings.apiKey')}</label>
        <input class="input" type="password" id="s-market-key" placeholder=""></div>
    </div>
    <div class="form-group" id="s-market-base-wrap" style="display:none"><label class="form-label">${t('settings.baseUrl')}</label>
      <input class="input" id="s-market-base" placeholder="https://api.example.com"></div>
    <button class="btn btn-primary" id="save-market" style="margin-top:var(--space-3)">${t('settings.saveMarket')}</button>
  </div></div>
  <div class="card"><div class="card-header"><div class="card-header-title">${t('settings.aiProvider')}</div></div><div class="card-body">
    <div class="form-row">
      <div class="form-group"><label class="form-label">Provider</label>
      ...
      <div class="form-group"><label class="form-label">${t('settings.apiKey')}</label>
        <input class="input" type="password" id="s-ai-key" placeholder=""></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">${t('settings.model')}</label>
        <input class="input" id="s-ai-model" placeholder="gemini-pro"></div>
      <div class="form-group" id="s-ai-base-wrap" style="display:none"><label class="form-label">${t('settings.baseUrl')}</label>
        <input class="input" id="s-ai-base" placeholder="https://api.example.com/v1"></div>
    </div>
    <button class="btn btn-primary" id="save-ai" style="margin-top:var(--space-3)">${t('settings.saveAI')}</button>
  </div></div></div>`;
}
...
function _indicatorsPanel() {
  ...
  return `<div class="card"><div class="card-header"><div class="card-header-title">${t('settings.indicatorsConfig')}</div></div>
    <div class="card-body">${rows}
      <button class="btn btn-primary" id="save-indicators" style="margin-top:var(--space-4)">${t('settings.saveIndicators')}</button>
    </div></div>`;
}
...
function _strategiesPanel() {
  ...
  return `<div class="card"><div class="card-header"><div class="card-header-title">${t('settings.strategiesConfig')}</div></div>
    <div class="card-body">${rows}
      <button class="btn btn-primary" id="save-strategies" style="margin-top:var(--space-4)">${t('settings.saveStrategies')}</button>
    </div></div>`;
}
...
function _riskPanel() {
  return `<div class="card"><div class="card-body">
    <div class="form-row">
      <div class="form-group"><label class="form-label">${t('settings.riskPerTrade')}</label>
        <input class="input" type="number" data-risk="riskPercent" value="2" min="0.1" max="100" step="0.1"></div>
      <div class="form-group"><label class="form-label">${t('settings.defaultSLMethod')}</label>
    ...
    <div class="form-row">
      <div class="form-group"><label class="form-label">${t('settings.atrPeriod')}</label>
        <input class="input" type="number" data-risk="atrPeriod" value="14" min="1"></div>
      <div class="form-group"><label class="form-label">${t('settings.atrMultiplier')}</label>
    ...
    <button class="btn btn-primary" id="save-risk" style="margin-top:var(--space-4)">${t('settings.saveRisk')}</button>
  </div></div>`;
}

function _dataPanel() {
  return `<div class="card"><div class="card-body">
    <p style="color:var(--text-muted);margin-bottom:var(--space-4)">${t('settings.dataHint')}</p>
    <div style="display:flex;gap:var(--space-3);flex-wrap:wrap">
      <button class="btn btn-primary" id="data-export"><svg style="width:16px;height:16px;vertical-align:middle;margin-inline-end:6px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/></svg>${t('settings.exportData')}</button>
      <button class="btn btn-outline" id="data-import-trigger"><svg style="width:16px;height:16px;vertical-align:middle;margin-inline-end:6px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="8 17 12 21 16 17"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.88 18.09A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.29"/></svg>${t('settings.importData')}</button>
      <input type="file" id="data-import-file" accept=".json" style="display:none">
      <button class="btn btn-danger" id="data-clear"><svg style="width:16px;height:16px;vertical-align:middle;margin-inline-end:6px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>${t('settings.clearAll')}</button>
    </div>
  </div></div>`;
}
...
const SettingsPage = {
  render(ctx) {
    return `<div class="page-header"><div class="page-header-left">
      ...
      <p class="page-header-subtitle">${t('settings.subtitle')}</p>
    </div></div>
    ${_settingsTabs()}`;
  },

  async init(ctx) {
    const { storage, eventBus, app } = ctx;
    ...
    if (themeSelect) themeSelect.value = savedSettings.theme || 'dark';

    const langSelect = document.querySelector('[data-setting="language"]');
    if (langSelect) langSelect.value = savedSettings.language || 'en';

    const demoToggle = document.querySelector('[data-setting="demoMode"]');
    if (demoToggle) demoToggle.checked = savedSettings.demoMode !== false;

    const symbolInput = document.querySelector('[data-setting="defaultSymbol"]');
    if (symbolInput) symbolInput.value = savedSettings.defaultSymbol || 'BTCUSDT';

    const tfSelect = document.querySelector('[data-setting="defaultTimeframe"]');
    if (tfSelect) tfSelect.value = savedSettings.defaultTimeframe || '1h';

    const autoRefreshToggle = document.querySelector('[data-setting="autoRefresh"]');
    if (autoRefreshToggle) autoRefreshToggle.checked = savedSettings.autoRefresh === true;

    const refreshInput = document.querySelector('[data-setting="refreshInterval"]');
    if (refreshInput) refreshInput.value = savedSettings.refreshInterval || 30;

    // Market provider — load saved values
    const savedMarketProvider = savedSettings.marketProvider || 'binance';
    const marketProviderSelect = $('s-market-provider');
    if (marketProviderSelect) marketProviderSelect.value = savedMarketProvider;

    const marketCreds = storage?.getCredentials?.(savedMarketProvider) || {};
    const marketKeyInput = $('s-market-key');
    if (marketKeyInput) marketKeyInput.value = marketCreds.apiKey || '';
    const marketBaseInput = $('s-market-base');
    if (marketBaseInput) marketBaseInput.value = marketCreds.baseUrl || '';
    if (marketProviderSelect?.value === 'custom') {
      $('s-market-base-wrap').style.display = '';
    }

    // AI provider — load saved values
    const savedAIProvider = savedSettings.aiProvider?.provider || 'gemini';
    const aiProviderSelect = $('s-ai-provider');
    if (aiProviderSelect) aiProviderSelect.value = savedAIProvider;

    const aiCreds = storage?.getCredentials?.(savedAIProvider) || {};
    const aiKeyInput = $('s-ai-key');
    if (aiKeyInput) aiKeyInput.value = aiCreds.apiKey || '';
    const aiModelInput = $('s-ai-model');
    if (aiModelInput) aiModelInput.value = savedSettings.aiProvider?.model || '';
    const aiBaseInput = $('s-ai-base');
    if (aiBaseInput) aiBaseInput.value = aiCreds.baseUrl || '';
    if (aiProviderSelect?.value === 'openai_compatible') {
      $('s-ai-base-wrap').style.display = '';
    }

    // Load risk settings
    const savedRisk = await storage?.getSetting?.('risk', {});
    if (savedRisk) {
      const riskFields = ['riskPercent', 'slMethod', 'atrPeriod', 'atrMultiplier', 'tpRatio1', 'tpRatio2', 'tpRatio3'];
      riskFields.forEach(key => {
        const el = document.querySelector(`[data-risk="${key}"]`);
        if (el && savedRisk[key] !== undefined) {
          el.value = savedRisk[key];
        }
      });
    }

    // Load indicator configs
    const savedIndicators = await storage?.getSetting?.('indicators', {});
    if (savedIndicators) {
      Object.entries(savedIndicators).forEach(([key, config]) => {
        const el = document.querySelector(`[data-indicator="${key}"]`);
        if (el) el.checked = config.enabled !== false;
        if (config.params) {
          Object.entries(config.params).forEach(([pKey, pVal]) => {
            const el = document.querySelector(`[data-param="ind_${key}_${pKey}"]`);
            if (el) el.value = pVal;
          });
        }
      });
    ...
    const savedStrategies = await storage?.getSetting?.('strategies', {});
    if (savedStrategies) {
      Object.entries(savedStrategies).forEach(([key, config]) => {
        const el = document.querySelector(`[data-strategy="${key}"]`);
        if (el) el.checked = config.enabled !== false;
        if (config.params) {
          Object.entries(config.params).forEach(([pKey, pVal]) => {
            const el = document.querySelector(`[data-param="strat_${key}_${pKey}"]`);
            if (el) el.value = pVal;
          });
        }
      });
    ...
    $('s-market-provider')?.addEventListener('change', e => {
      ...
      // Also load credentials for newly selected provider
      const creds = storage?.getCredentials?.(e.target.value) || {};
      const keyInput = $('s-market-key');
      if (keyInput) keyInput.value = creds.apiKey || '';
      const baseInput = $('s-market-base');
      if (baseInput) baseInput.value = creds.baseUrl || '';
    });
    $('s-ai-provider')?.addEventListener('change', e => {
      ...
      if (keyInput) keyInput.value = creds.apiKey || '';
      const baseInput = $('s-ai-base');
      if (baseInput) baseInput.value = creds.baseUrl || '';
      // Load model from settings
      const aiProv = app?.settings?.aiProvider;
      const modelInput = $('s-ai-model');
      if (modelInput && aiProv?.model) modelInput.value = aiProv.model;
    });
    ...
    async function saveGeneral() {
      ...
      if (app) {
        ...
        await storage?.setSetting?.('general', app.settings);

        // Apply theme immediately
        if (newSettings.theme) {
          document.documentElement.setAttribute('data-theme', newSettings.theme);
          const meta = document.querySelector('meta[name="theme-color"]');
          if (meta) meta.content = newSettings.theme === 'light' ? '#f8fafc' : '#0f1117';
        }
    ...
    async function saveMarket() {
      ...
      // Also save to app so it takes effect immediately
      if (app) {
        app.setMarketProvider(provider, { apiKey, baseUrl });
      }

      showToast(t('settings.marketSaved'), 'success');
    ...
    async function saveAI() {
      ...
      // Also save to app so it takes effect immediately
      if (app) {
        app.setAIProvider(provider, { apiKey, baseUrl, model });
      }

      showToast(t('settings.aiSaved'), 'success');
    ...
    async function saveIndicators() {
      ...
      document.querySelectorAll('[data-param^="ind_"]').forEach(el => {
        ...
        if (!config[indKey]) config[indKey] = { enabled: true };
        if (!config[indKey].params) config[indKey].params = {};
        config[indKey].params[paramKey] = parseFloat(el.value);
      });
      await storage?.setSetting?.('indicators', config);
      if (app) app.indicatorConfigs = config;
      showToast(t('settings.indicatorsSaved'), 'success');
    ...
    async function saveStrategies() {
      ...
      document.querySelectorAll('[data-param^="strat_"]').forEach(el => {
        ...
        if (!config[stratKey]) config[stratKey] = { enabled: true };
        if (!config[stratKey].params) config[stratKey].params = {};
        config[stratKey].params[paramKey] = parseFloat(el.value);
      });
      await storage?.setSetting?.('strategies', config);
      if (app) app.strategyConfigs = config;
      showToast(t('settings.strategiesSaved'), 'success');
    ...
    async function saveRisk() {
      ...
      document.querySelectorAll('[data-risk]').forEach(el => {
        ...
        risk[el.dataset.risk] = val;
      });
      await storage?.setSetting?.('risk', risk);
      showToast(t('settings.riskSaved'), 'success');
    ...
    importInput?.addEventListener('change', async (e) => {
      ...
      try {
        ...
        showToast(t('settings.imported', { imported: result?.imported ?? 0, skipped: result?.skipped ?? 0 }), 'success');
        eventBus?.emit?.('data:imported', result);
      } catch (err) {
        showToast(t('settings.importFailed') + err.message, 'error');
      }
      e.target.value = '';
    });
    ...
    $('data-clear')?.addEventListener('click', async () => {
      ...
      showToast(t('settings.cleared'), 'warning');
